...
class InTimeAccumulateWeightedAggregatorScaffold(Aggregator):
    def __init__(self, exclude_vars=None, aggregation_weights=None, expected_data_kind=DataKind.WEIGHT_DIFF):
        """Perform accumulated weighted aggregation and aggregates the SCAFFOLD global control parameters
        It computes
    ...
    def accept(self, shareable: Shareable, fl_ctx: FLContext) -> bool:
        ...
        if c_delta_para is None:
            self.log_error(
                fl_ctx,
                f"SCAFFOLD control updates missing for {client_name} at round: {contribution_round}",
            )
            return False
        ...
        for k in data.keys():
            ...
            c_weighted_value = c * aggregation_weight * float_n_iter
            current_total = self.total.get(k, None)
            c_current_total = self.c_total.get(k, None)
            if current_total is None:
                self.total[k] = weighted_value
                self.c_total[k] = c_weighted_value
                self.counts[k] = n_iter
            else:
                self.total[k] = current_total + weighted_value
                self.c_total[k] = c_current_total + c_weighted_value
    ...
    def aggregate(self, fl_ctx: FLContext) -> Shareable:
        ...
        c_aggregated_dict = dict()
        for k in self.total.keys():
            aggregated_dict[k] = self.total[k] / self.counts[k]
            c_aggregated_dict[k] = self.c_total[k] / self.counts[k]
        self.reset_stats()
        self.log_debug(fl_ctx, "End aggregation")

        dxo = DXO(data_kind=self.expected_data_kind, data=aggregated_dict)
        dxo.set_meta_prop(AlgorithmConstants.SCAFFOLD_CTRL_DIFF, c_aggregated_dict)
