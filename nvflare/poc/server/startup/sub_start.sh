...
start_fl() {
  ...
  lst=$SECONDS
((python3 -u -m flare.private.fed.app.server.server_train -m $DIR/.. -s fed_server.json --set secure_train=false config_folder=config host=${host} sp=${sp} 2>&1 & echo $! >&3 ) 3>$DIR/../pid.fl )
  pid=`cat $DIR/../pid.fl`
  echo "new pid ${pid}"
}
