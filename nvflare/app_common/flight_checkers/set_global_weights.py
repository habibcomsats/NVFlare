...
class SetGlobalWeights(FlightChecker):
    ...
    def create_task_data(self, task_name: str, fl_ctx: FLContext) -> Shareable:
        ...
        """
        # reset internal state in case this checker is used multiple times
        self.final_weights = None
        return Shareable()
    ...
    def process_client_report(
        ...
        Args:
            client: the client that submitted the report
            task_name: name of the task
            report: submitted data from the client
            fl_ctx: FLContext
