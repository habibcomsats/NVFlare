...
start_fl() {
  ...
  lst=$SECONDS
((python3 -u -m flare.private.fed.app.client.client_train -m $DIR/.. -s fed_client.json --set secure_train=false uid=${client} config_folder=config sp=${sp} 2>&1 & echo $! >&3 ) 3>$DIR/../pid.fl )
  pid=`cat $DIR/../pid.fl`
}
