...
considerations to make when designing and building your own [BLINDED] FLARE applications.

.. currentmodule:: flare.apis.fl_component.FLComponent

Define your logic in a subclass of FLComponent
----------------------------------------------
[BLINDED] FLARE has a componentized architecture - business logic is implemented by components that interact with each
other to get the job done. Most of your code, if not all, should be implemented as subclasses of the
:class:`FLComponent<flare.apis.fl_component.FLComponent>` class.
...
-----------------------
:class:`FLContext<flare.apis.fl_context.FLContext>` caters relevant data to your component's processing methods. All
...
[BLINDED] FLARE encourages data content to be self-sufficient in that it provides not only the content (e.g. values of
...
To enable easy adoption of this strategy, [BLINDED] FLARE provides a simple DXO API (Data eXchange Object) that you can use
...
purpose of security. In fact, [BLINDED] FLARE provided privacy protection techniques are all implemented as filters.

Be aware of the running environment
-----------------------------------
[BLINDED] FLARE is a messaging system running in a multithreaded environment. Care must be taken when programming
with [BLINDED] FLARE to ensure smooth operation of the system.
...
lucky. In addition to triggering the task abort signal, [BLINDED] FLARE also fires the special event EventType.ABORT_TASK!
...
.. tip::
    ...
    Firing an event from an event handler could be useful in some cases. For example, [BLINDED] FLARE's event system does
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Do not reuse :class:`Task<flare.apis.controller_spec.Task>` objects -- always create a new Task object when scheduling
...
Task props usage
^^^^^^^^^^^^^^^^
.. currentmodule:: flare.apis.controller_spec.Task
...
[BLINDED] FLARE's event system is like a pub-sub mechanism. The publisher fires an event, and subscribers handle the event.
...
for the internal use of [BLINDED] FLARE framework.
